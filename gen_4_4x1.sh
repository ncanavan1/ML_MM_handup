					...
					for l4 in ${list4}*.smi ; do
						...
						if ((l >= k )) ; then
							for l5 in ${list5}*.smi ; do
									...
									fn5=$(basename -- "$l5")
									while read lanth ; do
										fname=${lanth}_${fn1::-4}_${fn2::-4}_${fn3::-4}_${fn4::-4}_${fn5::-4}.smi			
									#	echo $fname
										cat 5_Lig_template | sed "s/L1/$(cat $l1)/g;s/L2/$(cat $l2)/g;s/L3/$(cat $l3)/g;s/L4/$(cat $l4)/g;s/L5/$(cat $l5)/g;s/Ln/${lanth}/g" > full/$fname
									done < ln_list
								fi
							((m++))
							done
